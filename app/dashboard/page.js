...
export default function DashboardPage() {
  ...
  // ── Fetch videos: DB first, live fallback ─────────────────────────────────
  const fetchVideos = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        sort, page: String(page), limit: '18',
      });
      ...
      // 2. DB empty or not set up yet — fetch live from YouTube
      // (only on page 1 and no search filter, since live has no pagination)
      if (page === 1 && !search) {
        ...
        // Apply client-side sort to live data
        const sortFns = {
        ...
        items = [...items].sort(sortFns[sort] || sortFns.trending_score);

        setVideos(items);
        setPagination({ page: 1, totalPages: 1, total: items.length });
        setDataSource('live');
      } else {
        // Search on live data — filter client-side from what we already have
        setVideos([]);
  ...
  async function handleDownloadCSV() {
    const source = dataSource === 'live' ? '/api/trends/live' : '/api/trends?limit=50&sort=trending_score';
    const res  = await fetch(source);
    const data = await res.json();
    const rows = data.data || [];
    const headers = ['title','channel_name','view_count','like_count','comment_count','trending_score','published_at'];
    const csv = [
      headers.join(','),
      ...
      <nav className="border-b border-gray-800 px-6 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 text-sm">
          {isPro ? (
            ...
            <Link href="/pricing" className="text-blue-400 hover:underline">Upgrade to Pro</Link>
          )}
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
        ...
        {dataSource === 'live' && !loading && (
          <div className="bg-blue-950/40 border border-blue-800 rounded-xl px-4 py-2.5 flex items-center gap-3 text-sm">
            ...
            <span className="text-blue-300">
              Showing <strong>live YouTube data</strong> — database will be populated after the first daily scrape runs.
            </span>
          </div>
        )}
        ...
        {upgradeMessage && !isPro && (
          <div className="bg-yellow-900/30 border border-yellow-700 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <button onClick={() => setUpgradeMessage(false)} className="text-gray-500 hover:text-gray-300 text-xs" aria-label="Dismiss">✕</button>
            </div>
          </div>
        )}
          ...
          <form onSubmit={handleSearchSubmit} className="flex gap-2 flex-1 min-w-0 max-w-md">
            ...
            {search && (
              <button type="button" className="btn-secondary px-3 shrink-0"
                onClick={() => { setSearch(''); setSearchInput(''); }} aria-label="Clear search">✕</button>
            )}
          </form>

          <div className="flex gap-2 items-center">
            <label htmlFor="sort" className="text-sm text-gray-400 shrink-0">Sort by:</label>
            ...
            {isPro && (
              <button onClick={handleDownloadCSV} className="btn-secondary text-sm px-3 py-2 shrink-0" title="Download CSV (Pro)">
                ⬇ CSV
              </button>
            )}
          </div>
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500">
          ...
          {!isPro && (
            <span className="ml-auto text-yellow-600">
              Free · {savedIds.size}/{FREE_SAVE_LIMIT} saves
            </span>
          )}
        </div>
        ...
        {error && !loading && (
          <div className="card border-red-900 bg-red-950/20 text-center py-8">
            ...
            <button onClick={() => fetchVideos(1)} className="btn-primary mt-4 text-sm px-4 py-2">
              Retry
            </button>
          </div>
        )}
        ...
        {!loading && !error && videos.length === 0 && (
          <div className="text-center py-16 text-gray-500">
            ...
            <button onClick={() => fetchVideos(1)} className="btn-secondary mt-4 text-sm px-4 py-2">
              Retry
            </button>
          </div>
        )}
